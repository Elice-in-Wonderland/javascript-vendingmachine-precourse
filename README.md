...
- 상단에 상품관리, 잔돈충전, 상품구매 탭이 존재한다.

- 각각의 탭 메뉴를 클릭하면 이동이 가능해야한다.

- 상품관리 탭
  - 상품을 추가할 수 있는 입력창과 상품 현황이 존재한다.
  - 최초에는 상품 현황이 비워진 상태로 보여진다.
  - 상품명, 가격, 수량을 입력해 상품을 추가할 수 있다.
    - 상품명은 고유해야 한다.
    - 상품 가격은 100원부터 시작하며 10원으로 나누어 떨어져야 한다.
    - 가격, 수량은 자연수여야 한다.
  - 상품이 추가되면 상품 현황에 추가되어야 한다.

- 잔돈충전 탭
  - 자판기에 동전을 충전할 수 있는 입력창이 존재한다.
  - 최초에는 동전 보유 현황의 동전 갯수는 0개이다.
  - 금액을 입력해 동전을 추가할 수 있다.
    - 금액은 자연수여야 한다.
    - 자판기 보유 금액은 {금액}원 형식으로 나타낸다.
    - 금액은 누적되어 충전되어야 한다.
  - 금액이 추가되면 보유 금액만큼의 동전이 무작위로 생성된다.
    - 동전의 개수는 {개수}개 형식으로 나타낸다.

- 상품구매 탭
  - 금액을 투입할 수 있는 입력창과 상품현황 그리고 잔돈버튼이 존재한다.
  - 최초에는 반한된 각 동전의 갯수는 0개이다.
  - 금액을 입력해 충전할 수 있다.
    - 금액은 10원으로 나누어 떨어지는 금액만 투입할 수 있다.
    - 자판기에 투입된 금액은 {금액}원 형식으로 나타낸다.
    - 금액은 누적되어 충전되어야 한다.
  - 상품 현황의 구매하기 버튼을 클릭하여 상품을 구매할 수 있다.
    - 수량이 0이라면 구매하기 버튼이 비활성화되어야 한다.
    - 구매시 수량이 하나씩 줄어들어야 한다.
    - 투입한 금액에서 상품의 가격만큼 줄어들어야 한다.
  - 잔돈을 반환할 수 있다.
