...
- 상단 탭
  ...
  - 상품 관리, 잔돈 충전, 상품 구매 버튼을 구현한다.
  - 버튼을 누르면 해당 탭으로 이동이 가능하게 한다.

- 상품 관리 탭

  - 상품명, 가격, 수량을 입력할 인풋 창과 추가하기 버튼을 구현한다.
  - 인풋 입력 후, 추가하기를 누르면 상품이 추가되게 한다.
  - 추가한 상품을 확인할 수 있게 구현한다.

- 잔돈 충전 탭

  - 충전할 금액을 입력할 인풋 창과 충전하기 버튼을 구현한다.
  - 금액 입력 후, 충전하기 버튼 누르면 금액이 충전되게 한다.
  - 화면에 동전 갯수가 출력되게 한다.
  - 보유 금액을 누적 충전할 수 있게 한다.
  - 추가 충전된 동전과 기존의 동전을 합친 동전의 갯수를 화면에 출력되게 한다.

- 상품 구매 탭

  - 투입할 금액을 입력할 인풋 창과 투입하기 버튼을 구현한다.
  - 인풋 입력 후, 투입하기 버튼을 누르면 금액이 충전되게 한다.
  - 충전된 금액이 화면에 출력되게 한다.
  - 금액이 누적으로 투입될 수 있게 한다.
  - 구매 가능한 상품들의 현황에 대한 정보를 출력할 표와 구매하기 버튼을 구현한다.
  - 구매하기 버튼을 누르면 해당 상품의 수량이 줄어들고, 투입금액이 차감되게 한다.
  - 품절된 상품의 구매하기 버튼을 disabled 시킨다.
  - 잔돈을 반환할 수 있는 버튼을 구현한다.
  - 잔돈 반환시 현재 보유한 최소 개수의 동전으로 반환되게 한다.
  - 잔돈을 반환할 수 없는 경우 반환 가능 금액만 반환되게 한다.
  - 반환 동전 내역이 화면에 출력되게 한다.
